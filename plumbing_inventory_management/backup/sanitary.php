...
$sql = "SELECT item, quantity, unit FROM sanitary;";
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        echo "<td>$unit</td>";
        echo "<td>
              <button class='consume-button' onclick=\"updateQuantity('consume', 'sanitary', '$item', $quantity)\">Consume</button>
              <button class='add-button' onclick=\"updateQuantity('add', 'sanitary', '$item', $quantity)\">Add</button>
              </td>";
        echo "</tr>";
    }
    echo "</table>";
