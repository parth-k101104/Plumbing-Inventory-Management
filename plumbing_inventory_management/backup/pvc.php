...
$sql = "SELECT item, quantity, unit FROM pvc;";
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        echo "<td>$unit</td>";
        echo "<td>
              <button class='consume-button' onclick=\"updateQuantity('consume', 'pvc', '$item', $quantity)\">Consume</button>
              <button class='add-button' onclick=\"updateQuantity('add', 'pvc', '$item', $quantity)\">Add</button>
              </td>";
        echo "</tr>";
    }
    echo "</table>";
